...
(3.4) $\operatorname{talk}_M:\mathbf e\rightarrow \mathbf t$

(3.5) $\text{(\#) A table talks.}$

(3.6) $(?)\ \exists x:\mathbf e.\ \operatorname{table}_M(x)\ \wedge\ \operatorname{talk}_M(x)$

(3.7) $(\#)\ \exists t:Table.\ talk(t)$
...
(3.10) $\text{quickly}_M:(\mathbf e\rightarrow \mathbf t)\ \rightarrow\ (\mathbf e\rightarrow \mathbf t)$
...
(3.13) $\text{some}:\Pi A:\mathsf{CN}.\,(A\rightarrow Prop)\ \rightarrow\ Prop$
...
(3.16) $(\#)\ \text{some}(Prop\rightarrow Student,\ \ldots)$
...
(3.17) $\text{EasyJet has transported 1 million passengers in 2010.}$  
（2010 年，易捷承运了一百万**乘客次**。）

(3.18) $\text{EasyJet has transported 1 million persons in 2010.}$  
（2010 年，易捷承运了一百万**人**。）
...
(3.19) $\text{John taught 500 students last year.}$  
（去年的“学生”可按**班级注册**计，跨班可重复。）

(3.20) $\text{1,000 students have applied for campus cards last year.}$  
（此处“学生”与“person”的辨识准则对齐，按人头计。）
...
(3.23) $W\&P:\ \textit{heavy book}$，其中 $\textit{heavy book} \;\coloneqq\; \Sigma x:Book.\; heavy(x)$
...
(3.27) $\exists b:Book.\ read(j,b)$

(3.28) $\exists h:Human.\ read(h,W\&P)$
...
(3.30) $Man \leq Human$  （男人是人）

(3.31) $\Sigma x:Book.\ heavy(x) \leq Book$  （重书是书）
...
(3.32) $(Human \to Book \to Prop)\ \leq\ (Man \to \Sigma x:Book.\ heavy(x) \to Prop)$
...
(3.33) $\Sigma x:A.\ P(x)\ \leq_{\pi_1}\ A$
...
(3.36) $run_{1}: Human \rightarrow Prop$

(3.37) $run_{2}: Human \rightarrow Institution \rightarrow Prop$
...
(3.39) $[[\text{John runs quickly}]]\;=\; quickly(\text{run},j)\;=\; quickly(run_{1},j)$
...
(3.41) $\text{Julie enjoyed (started/finished) a book.}$

(3.42) $\text{Julie enjoyed doing something with (e.g. reading, writing, …) a book.}$

(3.43) $\text{Julie enjoyed a book.}$
...
(3.46) $\text{Julie enjoyed reading a book.}$

(3.47) $\exists x:Book.\ enjoy(j,reading(x))$

当然，强制并非唯一：也可能是 “enjoy writing a book”。不同强制的**作用域**可彼此交错（overlap），如（3.48）所示：

(3.48) $\text{Julie just started War and Peace,}\\
\text{which Tolstoy finished after many years of hard work.}\\
\text{But that will not last because she never gets through long novels.}$
...
进一步，引入**参数化强制**[^11]
$$
(3.49)\qquad Book \leq_{c(h)} Evt_{A}(h)\quad (h:Human),
$$
并定义
$$
(3.50)\qquad c(h,b)=
...
(3.63) (\#) $(j:Human)\ \wedge\ \operatorname{happy}(j)$
...
(3.65) $p_{Human}(j)\ \wedge\ \operatorname{happy}(j)$
...
(3.72) $\mathrm{\small IS}(Table, j)\ \Rightarrow\ \operatorname{happy}(m)$
...
(3.76) $\mathrm{\small IS}(Student, j)\ \wedge\ \operatorname{happy}(j)$

(3.77) $\neg\,\mathrm{\small IS}(Student, j)$

(3.78) $\mathrm{\small IS}(Student, j)\ \Rightarrow\ \operatorname{happy}(j)$
...
(3.83) (\#) $\exists x:Table.\ \operatorname{talk}(x)\ \Rightarrow\ \operatorname{surprised}(m)$

(3.84) $\exists x:Table.\ \mathrm{\small DO}(\operatorname{talk},x)\ \Rightarrow\ \operatorname{surprised}(m)$

从技术上，$\mathrm{\small IS}$ 与 $\mathrm{\small DO}$ 可统一经由一个更基本的算符 $\mathrm{\small NOT}$ 来定义。其类型与定义如下：

(3.85) $\mathrm{\small NOT}:\ \Pi X:\mathsf{CN}\ \Pi p:X\!\to\!Prop\ \Pi Y:\mathsf{CN}\ \Pi y:Y.\ Prop$

(3.86) 对任意 $X:\mathsf{CN},\ y:B$，定义
\[
\mathrm{\small IS}(X,y)\;=\;\neg\,\mathrm{\small NOT}\bigl(X,\ p_X,\ B,\ y\bigr),
\]
其中 $p_X$ 为 (3.64) 的恒真谓词。

(3.87) 对任意 $p:A\!\to\!Prop,\ y:B$，定义
\[
...
否定/剥夺型（privative）修饰使结果**不属于**原名词类，如“fake gun（假枪）”并不属于“gun”。非承诺型（non-committal）则不对是否属于原类作承诺，如“alleged criminal（被指称的罪犯）”既可能是罪犯，也可能不是[^19]。
...
(3.89) $\text{black cat} = \Sigma x: Cat.\ \text{black}(x)$

关键在于：子类型会**经由类型构造器**（如 $\Sigma$）向上传播。一只“black cat”当然也是“black object”。这对应于子类型关系 (3.91)：若 $Cat \leq Object$，则通过 $\Sigma$ 的传播可得

(3.91) $\Sigma x: Cat.\ \text{black}(x) \leq \Sigma x: Object.\ \text{black}(x)$

[^20]: 子类型随构造器传播可由推导规则形式化（Luo 1999）。以 $\Sigma$ 为例（使用 $\mathrm{LF}_{\Delta}$ 记法）：
$$
\frac{\Gamma \vdash_{\Delta} A \leq_{c} A'\quad
      \Gamma, x\!:\!A \vdash_{\Delta} B(x) \leq_{c'[x]} B'(c(x))}
     {\Gamma \vdash_{\Delta} \Sigma(A,B) \leq_{d_{\Sigma}} \Sigma(A',B')}
$$
其中 $d_{\Sigma}$ 把 $(a,b)$ 映到 $\bigl(c(a),\ c'[a](b)\bigr)$。形式化地，对任意 $z:\Sigma(A,B)$，
$d_{\Sigma}(z)=\bigl(c(\pi_1(z)),\ c'[\pi_1(z)](\pi_2(z))\bigr)$。
...
(3.92) $\text{A black cat is a cat.}$

(3.93) $\operatorname{Adj}[\mathrm{N}] \Rightarrow \mathrm{N}$

(3.94) $\Sigma x:\ [[N]].\ [[Adj]](x)\ \leq\ [[N]]$
...
(3.95) $\text{A black cat is black.}$

(3.96) $\operatorname{Adj}[\mathrm{N}] \Rightarrow \operatorname{Adj}$

(3.97) 若 $p:\Sigma x:[[N]].[[Adj]](x)$，则 $[[Adj]]\bigl(\pi_1(p)\bigr)$ 为真
...
(3.98) $\text{Most persons who traveled by plane are happy.}$
...
(3.103) $\text{small elephant}=\Sigma x:\ Elephant.\ \text{small}(\!Elephant\!)(x)$

(3.104) $\text{small animal}=\Sigma x:\ Animal.\ \text{small}(\!Animal\!)(x)$
...
(3.105) $\operatorname{Adj}[N] \Rightarrow N$
...
“fake（假）”“imaginary（虚构的）”“fictitious（杜撰的）”等通常被归为**欠性**（privative）形容词。直观上，人们常写作

(3.106) $\operatorname{Adj}[N] \Rightarrow \neg N$

例如：

(3.107) $\text{A fake gun is not a (real) gun.}$

不过，欠性是否真的总意味着 (3.106)，并无共识。Partee (2007, 2010) 主张，像 “fake” 的语义更适合与**类型转换/语言强制**联动理解：在 (3.108)(3.109) 的语境中，“fur” 的涵义似应扩展为“真皮毛 + 假皮毛”的并集。

(3.108) *I don't care whether that fur is fake fur or real fur.*

(3.109) *I don't care whether that fur is fake or real.*

与此思路一致，Luo (2011a) 在 MTT-语义中提出用**无交并和类型（disjoint union）**来建模欠性修饰（见 §2.2.3），随后在 Chatzikyriakidis & Luo (2013, 2017a) 进一步发展。以 “fake” 修饰 “gun” 为例：
...
(3.111) $\operatorname{real}_g(\operatorname{inl}(r))=\mathbf{true},\quad \operatorname{real}_g(\operatorname{inr}(f))=\mathbf{false}$

(3.112) $\operatorname{fake}_g(\operatorname{inl}(r))=\mathbf{false},\quad \operatorname{fake}_g(\operatorname{inr}(f))=\mathbf{true}$

再把“fake/real”做成在 $\mathsf{CN}$ 上的 $\Pi$ 多态家族（与细分型处理保持一致）：

(3.113) $\textit{fake},\ \textit{real}:\Pi A:\mathsf{CN}.\ (A\rightarrow Prop)$

对具体 $A$ 取实例。若 $A$ 由上面的 $G$ 解释，则

(3.114) $\textit{fake}(G)=\operatorname{fake}_g\quad$ (3.115) $\textit{real}(G)=\operatorname{real}_g$

据此，用 $\Sigma$-类型形成修饰后的 CN：

(3.116) $\text{fake gun}=\Sigma g:\ G.\ \textit{fake}(G,g)$

(3.117) $\text{real gun}=\Sigma g:\ G.\ \textit{real}(G,g)$

该设置给出预期推论。例如：

(3.118) *That gun is either real or fake.*  
(3.119) *A fake gun is not a (real) gun.*

可分别解释为：

(3.120) $\operatorname{real}(G,g)\ \vee\ \operatorname{fake}(G,g)$（其中 $g:G$ 解释“that gun”）

(3.121) $\forall f:\bigl[\Sigma g:\ G.\ \operatorname{fake}(G,g)\bigr].\ \neg\operatorname{real}(G,f)$
...
(3.123) $\text{John is an alleged murderer.}$  
(3.124) $\text{Mary hates every alleged murderer.}$

我们形式化为：

(3.125) $\exists h:\ Human.\ H_{h,\ \textit{alleged}}\!\big(\mathrm{\small IS}(Murderer,\ j)\big)$

(3.126) $\forall x:\ Human.\ \Big[\exists h:\ Human.\ H_{h,\ \textit{alleged}}\!\big(\mathrm{\small IS}(Murderer,\ x)\big)\Big]\ \Rightarrow\ hate(m,\ x)$
...
(3.127) $H_{h,\ Adj}:\ Prop \rightarrow Prop$
