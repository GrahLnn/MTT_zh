...
因此，将 CN 形式化为**等价化集合（setoid）**最为贴切：$(A,\!\sim)$，其中 $A$ 是类型，$\sim$ 是刻画该 CN 辨识准则的等价关系。该思路在 Luo（2012a）系统阐述。实践上，大多数现象可安全忽略多重辨识准则的复杂性，直接把 CN 当作类型使用（Chatzikyriakidis & Luo 2018）[^6]。
