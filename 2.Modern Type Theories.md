...
$$
\tag{2.3} x_{1}:A_{1},\ x_{2}:A_{2},\ \ldots,\ x_{n}:A_{n}
$$
其中 \(x_i\) 是类型 \(A_i\) 的变量，且 \(A_i\) 可依赖先前变量 \(x_1,\ldots,x_{i-1}\)。例如

$$
\tag{2.4} x:Man,\ \ y:handsome(x)
$$
...
$$
\tag{2.5} c_{1}:A_{1},\ c_{2}:A_{2},\ \ldots,\ c_{n}:A_{n}
$$
...
$$
\tag{2.6} x_{1}:A_{1},\ldots,x_{n}:A_{n}\ \vdash_{\,c_{1}:A_{1}',\ldots,c_{m}:A_{m}'}\ a:A
$$
...
\[
\tag{2.12}\text{If a man is handsome, he is not ugly.}\\如果一个男人长得帅，他就不丑。
\]
可形式化为
\[
...
\[
\tag{2.23}\text{Some students spoke.}\\一些学生发言了。
\]
可解释为
\[
...
$$
\tag{2.38} j:Man \le Human
$$

$$
\tag{2.39} \text{John shouts.}\\约翰大声喊叫。
$$

\[
\tag{2.40}\operatorname{shout}(j):Prop
\]
...
\[
\tag{2.47} \text{The ham-sandwich left without paying the bill.}\\那位三明治吃客离开时没有付账。
\]
在餐馆场景的签名中加入
\[
