...
[^24]: 从技术上讲，强归一化性质表明，每一个从良构类型项开始的计算都会终止。这是一个基本的性质，保证了证明系统的良好行为，特别是，它通常暗示了所关注的类型理论的逻辑一致性。
[^25]: 对元理论的详细研究超出了本书的范围。对此感兴趣的读者可以参考，例如，Lungu和Luo（2018）的作品，以及Lungu的博士论文（Lungu 2018）以获取详细信息，其中Luo等人（2012）的保守性结果已经扩展到带有签名的计算。
