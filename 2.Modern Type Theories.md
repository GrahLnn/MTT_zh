...
[^4]: 形式上，变量可由量化或 \(\lambda\) 抽象出去；如（见 §2.3.1 与附录 A3.1）  
\[
\frac{\Gamma \vdash_{\Delta} A:\text{Type}\quad \Gamma,x:A \vdash_{\Delta} P:\text{Prop}}
     {\Gamma \vdash_{\Delta} \forall x:A.\ P:\text{Prop}}
\]
从（2.4）出发，连用两次即可得到 \(\forall x:Man\,\forall y:handsome(x).\ \neg ugly(x)\)。
