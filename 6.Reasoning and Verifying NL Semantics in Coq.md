...
\[
Definition\ \mathrm{ident} : \text{term}_1 := \text{term}_2
\tag{6.1}
\]
...
以 $\mathsf{HUMAN}$ 为例：其域为 `Human`，同一判准为其上的等价关系：
...
如第 5.3.1 节所述，“Man” 的同一判准从 “Human” 继承。先以 `AIC_Man` 指定其在 `Human`-等价之下判同（省略自反/对称/传递证明，见附录 A7.9）：
...
把 “book” 的两种同一判准分别立起后，即可证明双重区别性目标（略去若干词法与量词展开的样板）：
